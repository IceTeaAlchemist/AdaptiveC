...
var rec_flag = false;
      ...
      var g = new Dygraph(document.getElementById("div_g"), data,
                          {
                                labels: ['Time','F','Temp', 'Temp from Fluor'],
                                series: {
                                      'F': {
                                            axis:'y1'
                                      },
...
function checkFile() {
      if(rec_flag) {
                  ...
                  $.ajax({
                        method: "POST",
                        url: "readfromfile3.php",
                        data: {mode: "2"},
                        dataType: 'json',
                        success:function(result){
                        y = parseFloat(result[0]);
                        temp = parseFloat(result[2]);
                        fluortemp = parseFloat(result[1]);
                        }
            });
...
function savemyCSV() {
      ...
      data.forEach(function(dataArray, index) {
            ...
            dataString = diff_time.toFixed(0) + ',' + dataArray[1].toFixed(3) + ',' + dataArray[2].toFixed(3);
      ...
      {    
            ...
            {
                  rec_flag = false;
                  rec_startstop.textContent= "Start Recording";
            }
            else
            {
                  t = new Date();
